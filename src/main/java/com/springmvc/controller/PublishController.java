...
public class PublishController {
    ...
    public ModelAndView publish(HttpSession session){
        ...
        if(user!=null){
            ...
            if (catelogs == null || catelogs.isEmpty()) {
                String[] defaults = {"数码电子", "书籍教材", "日用百货", "运动出行", "其他"};
                for (String name : defaults) {
                    Catelog c = new Catelog();
                    c.setName(name);
                    c.setNumber(0);
                    c.setStatus((byte)1);
                    catelogService.insert(c);
                }
                catelogs = catelogService.getAllCatelogByStatus((byte)1);
            }
            //返回分类信息
    ...
    public Map<String,Object> uploadFile(HttpSession session,HttpServletRequest request, @RequestParam("info") MultipartFile fileName) throws IllegalStateException, IOException{
        ...
        if(fileName!=null && oldFileName!=null && oldFileName.length()>0){
            ...
            //新图片
            String url = uploadFile.getPath() + File.separator + newFileName;
