...
<div class="x-nav">
    ...
    <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"  href="/admin/good" title="刷新"><i class="layui-icon" style="line-height:30px">ဂ</i></a>
</div>
<div class="x-body">
    <form class="layui-form x-center" action="/search/searchgood" style="width:500px" type="post">
        <div class="layui-form-pane">
            <div class="layui-form-item">
                <div class="layui-input-inline" style="width:400px">
                    <input type="text" name="keyword"  placeholder="搜索闲置" autocomplete="off" class="layui-input">
                </div>
    ...
    <div style="height: 30px; background-color: #f2f2f2;line-height: 30px">
        <button class="layui-btn x-right" style="top:3px;" >共有数据：${fn:length(GoodsResult)} 条</button>
    </div>
    <table class="layui-table">
        ...
        <tbody id="x-img">
        <c:forEach var="item" items="${GoodsResult}">
            <tr>
                <td>${item.goods.id}</td>
                <td>${item.catelog.name}</td>
                <td>${item.user.username}</td>
                <td>${item.goods.name}</td>
                <td align="left">
                    <c:forEach var="image" items="${item.images}">
                        <img src="<%=basePath%>files/web/${image.imgUrl}" alt="${item.goods.name}" width="56" height="56">
                    </c:forEach>
                </td>
                <td >${item.goods.polishTime}</td>
                <td >${item.goods.describle}</td>
                <td class="td-status">
                    ...
                    <c:if test="${item.goods.status eq 0 || item.goods.status eq 2}">
                        <span class="layui-btn layui-btn-disabled layui-btn-mini">
                            已下架
                        </span>
                    </c:if>
                    <c:if test="${item.goods.status eq 3}">
                        <span class="layui-btn layui-btn-disabled layui-btn-mini">
                            已售出
                        </span>
                    </c:if>
